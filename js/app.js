requirejs(
  ...
  function ( App ) {
    ...
    // Settings inputs (inline in QBasic code)
    var gravityInput = document.getElementById( 'setting_gravity' );
    var explosionInput = document.getElementById( 'setting_explosion' );
    var speedInput = document.getElementById( 'setting_speed' );

    gravityInput.addEventListener( 'input', function () {
      var val = parseFloat( this.value );
      if ( !isNaN( val ) ) window.gameSettings.gravity = val;
    });

    explosionInput.addEventListener( 'input', function () {
      var val = parseInt( this.value );
      // Direct control: bigger number = bigger crater
      if ( !isNaN( val ) && val > 0 ) window.gameSettings.explosionSize = val;
    });

    speedInput.addEventListener( 'input', function () {
      var val = parseFloat( this.value );
      if ( !isNaN( val ) ) window.gameSettings.bananaSpeed = val;
    });
